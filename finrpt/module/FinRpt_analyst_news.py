...
class FinRpt_analyst_news:
    ...
    def run(self, date, stock_code=None, company_name=None):
        ...
        run_path = os.path.join(self.save_path, stock_code + "_" + date + "_" + self.model_name + "_analyst_news")
        ...
        key_news = data["news"]
        key_news = sorted(key_news, key=lambda x: x['news_time'], reverse=True)
        data["analyze_news"] = random.sample(key_news, min(10, len(key_news)))
