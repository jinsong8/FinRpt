...
class RiskAssessor(BaseModel):
    ...
    def run(self, data, run_path):
        ...
        risk_prompt = "分析日期: " + data["date"] + "\n\n" + \
            "公司名称: " + data["company_name"] + "\n\n" + \
            "股票近期总结信息:\n" + advisor_prompt + "\n\n" + \
            "股票半年报或年报:\n" + report_prompt + "\n\n" + \
            self.system_prompt
        logger.debug("<<<Risk Prompt>>>\n" + risk_prompt)
        data['save']['risk_prompt'] = risk_prompt
        response, response_json = self.model.json_prompt(risk_prompt)
        logger.debug('<<<response_json>>>\n' + str(response_json))
        data['save']['risk_response'] = json.dumps(response_json, ensure_ascii=False)
